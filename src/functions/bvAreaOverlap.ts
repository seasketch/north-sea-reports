...
export async function bvAreaOverlap(
  sketch: Sketch<Polygon> | SketchCollection<Polygon>
): Promise<ReportResult> {
  const box = sketch.bbox || bbox(sketch);
  const metricGroup = project.getMetricGroup("bvAreaOverlap");
...
export default new GeoprocessingHandler(bvAreaOverlap, {
  title: "bvAreaOverlap",
  description: "Calculate sketch overlap with biological value polygons",
  executionMode: "async",
  timeout: 600,
  requiresProperties: [],
});
