...
export async function suitabilityAreaOverlap(
  sketch: Sketch<Polygon> | SketchCollection<Polygon>
): Promise<ReportResult> {
  const box = sketch.bbox || bbox(sketch);
  const metricGroup = project.getMetricGroup("suitabilityAreaOverlap");
...
export default new GeoprocessingHandler(suitabilityAreaOverlap, {
  title: "suitabilityAreaOverlap",
  description:
    "Calculate sketch overlap with benthic habitat suitability polygons",
  executionMode: "async",
  timeout: 600,
  requiresProperties: [],
});
