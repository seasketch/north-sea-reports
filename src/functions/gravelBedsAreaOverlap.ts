...
export async function gravelBedsAreaOverlap(
  sketch: Sketch<Polygon> | SketchCollection<Polygon>
): Promise<ReportResult> {
  const box = sketch.bbox || bbox(sketch);
  const metricGroup = project.getMetricGroup("gravelBedsAreaOverlap");
...
export default new GeoprocessingHandler(gravelBedsAreaOverlap, {
  title: "gravelBedsAreaOverlap",
  description: "Calculate sketch overlap with gravel bed polygons",
  executionMode: "async",
  timeout: 600,
  requiresProperties: [],
});
